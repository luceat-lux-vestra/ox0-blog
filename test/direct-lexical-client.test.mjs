...
test('create/update send direct lexical without source=html conversion', async () => {
  const seen = [];
  ...
  const client = new GhostAdminClient({
    ...
    fetchImpl: async (url, options) => {
      ...
      seen.push({ method: options.method, source: parsed.searchParams.get('source'), saveRevision: parsed.searchParams.get('save_revision'), body });
