...
- First publication uses a staged production path: exact `main` source -> optional/read-only plan -> explicit managed draft -> fresh publish plan -> explicit production publish. Draft promotion requires exact-current managed drafts (plus exact-current published no-op siblings when recovering a partial multi-locale promotion), `reuse`-only local body assets, and only draft-to-published `status-update` / published `noop` Ghost operations.
- An already-published Article revision is different: every required locale must remain a uniquely managed `published` post, and fresh production planning may `update` stale published projections in place or `noop` exact-current siblings. It must never unpublish them merely to stage a revision. New local resource publication/feature-image upload may occur only as part of this explicitly authorized published-revision operation and remains subject to fresh source/resource/Ghost guards.
- The production mode chosen by the fresh authorized plan (`draft-promotion` or `published-revision`) is pinned across the publication library's initial and refreshed pre-mutation plans. A mode change fails closed.
...
- Live verification for this personal blog uses temporary **draft-only** Article projections on the configured Ghost instance. Evidence-only verification must never publish a temporary post merely to prove the workflow.
...
GitHub only dispatches a new `workflow_dispatch` workflow after that workflow exists on default branch. Before then, live exact-candidate evidence may use `npm run verify:article-live-draft` on the real Ghost instance because the verifier is hard-coded to create temporary drafts only and clean them up. Do not misrepresent a draft-verifier PASS as proof of production publication or GitHub workflow platform wiring.
